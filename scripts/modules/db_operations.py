...
def insert_referral_college_professor(conn, student_id, college_id, name, phone):
    """Insert data into vg_prod.referral_college_professor table if name or phone is provided."""
    try:
        # Validate inputs
        ...
        query = text("""
        INSERT INTO vg_prod.referral_college_professor (student_id, college_id, name, phone)
...
def insert_student_education(conn, student_id, education_course_id, subject_id, interest_subject_id, college_id, university_id, college_location_id):
    """Insert data into vg_prod.student_education table."""
    try:
        # Validate inputs
        if student_id is None:
            raise ValueError("student_id cannot be None")
        ...
        # Validate subject_id as a list of integers or None
        ...
        # Convert subject_id list to PostgreSQL array format (e.g., '{1,2,3}' or NULL)
        subject_id_array = f"{{{','.join(str(id) for id in subject_id)}}}" if subject_id and len(subject_id) > 0 else None

        query = text("""
        INSERT INTO vg_prod.student_education (student_id, education_course_id, subject_id, interest_subject_id, college_id, university_id, college_location_id)
        VALUES (:student_id, :education_course_id, :subject_id, :interest_subject_id, :college_id, :university_id, :college_location_id)
        """)
        conn.execute(query, {
            "student_id": student_id,
        ...
        raise  # Propagate the error for better debugging
...
def insert_student_registration(conn, student_id: int, form_details: dict, submission_timestamp: Union[datetime.datetime, str]):
    """Insert data into vg_prod.student_registration_details table."""
    try:
        if student_id is None:
            raise ValueError("student_id cannot be None")
        ...
        expected_keys = {"motivation", "problems", "new_university_name", "New_College_Name", "Currently_Pursuing_Year", "Currently_Pursuing_Degree"}
        ...
        query = text("""
            INSERT INTO vg_prod.student_registration_details (student_id, form_details, registration_date)
